...
document.getElementById("result-btn").addEventListener("click", function() {
  ...
  if (selectedOption && startCoords && endCoords) {
    ...
    if (mode === 'quietest') {
      var destinationUrl = '/quietest.html';
    } else if (mode === 'shortest') {
      var destinationUrl = '/shortest.html';
    }

    // Send mode along with start_coords and end_coords
    var data = {
      'start_coords': startCoords,
      'end_coords': endCoords,
      'mode': mode
    ...
    // Alert the user to select mode and click start and end points on the map
    window.alert('Please select mode and click start and end points on the map.');
  }
});
